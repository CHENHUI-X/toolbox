...
9. **WeChat can't receive files/images via `MEDIA:` paths or raw URLs — they arrive as unopenable text links.** User correction: "打不开这个文件，怎么被当做链接发过来的？". Fix: upload to a free file host (uguu.se works; 0x0.st/catbox/transfer.sh/file.io/tmpfiles.org all failed) and send the direct download URL as plain text. **Always verify the link serves raw bytes (`curl -sL | file`) before sending.** See `references/wechat-file-delivery.md` for the recipe and failed-service table.
10. **When 妈妈 sends an image and vision_analyze fails** (custom providers may reject image content, e.g. `unknown variant 'image_url', expected 'text'`), fall back to local tesseract OCR for text-bearing images: `apt-get install -y tesseract-ocr tesseract-ocr-chi-sim`, then `tesseract /abs/path/img.jpg /tmp/out -l chi_sim+eng`. Preprocess photos (grayscale + 2x upscale + contrast) for better results. If OCR returns empty, it's a photo with no text — ask 妈妈 to describe it instead of looping OCR variants. See `references/wechat-image-ocr-fallback.md`.
