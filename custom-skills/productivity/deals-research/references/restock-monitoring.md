...
用户质疑某条推荐不可买时，不要辩解——立刻逐款点进详情页/购物车验证并把结果（含售罄实锤）回给用户，同时撤掉清单里的死货。验证结果反哺本清单：同商家/同专区再次推荐前先复查库存（例：RackNerd 黑五专区列表页 16 个 Order Now，详情页全 Out of Stock，仅 shared hosting 有货；常规款 /store/kvm-vps 才有真货）。
